    ...
    switch (c) {
      ...
      case '?':
        printf ("Options:\n"
                "  v   : version\n"
                "  h|? : this message\n"
                "  R   : use RMAT from SSCA2 (default: Kronecker generator)\n"
                ...
                "        Note: Setting 3 of A,B,C,D requires the arguments to\n"
                "        sum to at most 1.  Otherwise, the parameters are\n"
      ...
      case 'd':
        ...
        break;
      case 'p':
        tmp_dump_path = strdup(optarg);
        if (!tmp_dump_path) {
          fprintf (stderr, "Cannot copy tmp dump path name.\n");
          err = 1;
        }
        break;
      case 't': {
