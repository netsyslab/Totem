...
__device__ __host__ inline bitmap_word_t bitmap_bit_mask(id_t bit) {
  return (((bitmap_word_t)1) << (bit % BITMAP_BITS_PER_WORD));
}

__device__ __host__ inline size_t bitmap_bits_to_bytes(id_t len) {
...
*/
inline bool bitmap_set_cpu(bitmap_t map, id_t bit) {
  bitmap_word_t mask = bitmap_bit_mask(bit);
  return !(__sync_fetch_and_or(&(map[bit / BITMAP_BITS_PER_WORD]), 
                               mask) & mask);
}

__device__ inline bool bitmap_set_gpu(bitmap_t map, id_t bit) {
  bitmap_word_t mask = bitmap_bit_mask(bit);
...
*/
__host__ __device__ inline bool bitmap_is_set(bitmap_t map, id_t bit) {
