 ...
 public:
  ...
  error_t TestGraph(vid_t src) {
    if (_graph500_param->attr) {
      ...
      _graph500_param->attr->pull_msg_size =
        (sizeof(vid_t) * BITS_PER_BYTE) + 1;
      _graph500_param->attr->alloc_func = g500s_stepwise_alloc;
      _graph500_param->attr->free_func = g500s_stepwise_free;
      if (totem_init(_graph, _graph500_param->attr) == FAILURE) {
        return FAILURE;
      }
      error_t err = g500s_stepwise_hybrid(src, _tree);
      totem_finalize();
      return err;
    }
