 ...
 */
PRIVATE error_t parse_metadata(FILE* file_handler, uint32_t* vertex_count, 
                               uint32_t* edge_count, bool* directed) {
...
error_t graph_initialize(const char* graph_file, bool weighted, 
  ...
  assert(mygraph);
  mygraph->vertices = (uint32_t*)mem_alloc((vertex_count + 1) * 
                                           sizeof(uint32_t));
  mygraph->edges    = (uint32_t*)mem_alloc(edge_count * sizeof(uint32_t));
  mygraph->weights  = weighted? 
    (int32_t*)mem_alloc(edge_count * sizeof(uint32_t)): NULL;
