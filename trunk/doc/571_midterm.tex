...
\todo{{\bf Graph algorithms on GPUs}}
...
\todo{mention that Pregel adopts a BSP model, and our intuition is that this model may serve well the targeted GPU-based platform.}

\todo{Google stuff? Map-reduce to process graphs?}
...
Third, and most importantly, we implemented three graph algorithms on the GPU: breadth-first search, single source shortest path and PageRank. \todo{The rest of this section discusses in more detail the implementation?}
...
\section{Future Work}
\label{sec:future}
\todo{the genome assembly related text is dumped here for now, we should clean this up}
